...
function ScrollAnime() {
    ...
    function callAnimeForLoop() {
        ...
        sectionArr.forEach((val,i) => {
            if (offset.top > innHeight*(i+1) && offset.top < innHeight * (i+2)){
                let dotheAnime = ()=>{
                ...
                switch (val.animeName) {
                    ...
                    case "clipPath":
                        if(i==sectionArr.length-1){
                            dotheAnime = ()=>{
                                document.querySelector(`#${val.secClass}`).style.clipPath = `inset(0% 0px 0px)`;
                            }
                        }else if(i==sectionArr.length-2){
                            dotheAnime = ()=>{
                                document.querySelector(`#${val.secClass}`).style.clipPath = `inset(0% 0px 0px)`;
                                document.querySelector(`#${sectionArr[i+1].secClass}`).style.clipPath = `inset(${100 - calcInterPercentage(i+1)}% 0px 0px)`;
                            }
                        }else{
                            dotheAnime = ()=>{
                                document.querySelector(`#${val.secClass}`).style.clipPath = `inset(0% 0px 0px)`;
                                document.querySelector(`#${sectionArr[i+1].secClass}`).style.clipPath = `inset(${100 - calcInterPercentage(i+1)}% 0px 0px)`;
                                document.querySelector(`#${sectionArr[i+2].secClass}`).style.clipPath = `inset(100% 0px 0px)`;
                            }
                        }
    ...
    return (
        <>
            ...
            <section className={cx('section section-colors theme-dark')}>
                ...
                <section className={cx('black', 'bigword', 'wipe', { 'active': topnumber == 5 })} id="black" >
                    <div className={cx('wipe-content')}>
                        <p>有黑色</p>
                    </div>

                </section>
                <section className={cx('red', 'bigword', 'wipe', { 'active': topnumber == 6 })} id="red" >
                    <div className={cx('wipe-content')}>
                        <p>也有紅色</p>
                    </div>
                </section>
